export function getErrorMessage(code: string, translations: Record<string, any>): string {
  const errorCodes: Record<string, string> = {
    ...
    'user_invalid_old_password': translations.authErrorCodes?.invalidOldPassword || 'Invalid old password',
    'password_mismatch': translations.authErrorCodes?.passwordMismatch || 'Passwords do not match',
    'missing_refresh_token': translations.authErrorCodes?.missingRefreshToken || 'Missing refresh token',
    ...
    'validation_error': translations.backendErrorCodes?.validationError || 'Validation error occurred',
    ...
    'version_already_exists': translations.depexErrorCodes?.versionAlreadyExists || 'Version already exists',
...
export function getSuccessMessage(code: string, translations: Record<string, any>): string {
  const successCodes: Record<string, string> = {
    ...
    'user_created_successfully': translations.authSuccessCodes?.signupSuccess || 'Account created successfully',
    ...
    'change_password_success': translations.authSuccessCodes?.changePasswordSuccess || 'Password changed successfully',
    'account_exists_success': translations.authSuccessCodes?.accountExistsSuccess || 'Account check completed',
    'token_verification_success': translations.authSuccessCodes?.tokenVerificationSuccess || 'Token is valid',
    'refresh_token_success': translations.authSuccessCodes?.refreshTokenSuccess || 'Token refreshed successfully',
    
    // Depex/Graph success codes
    'get_repositories_success': translations.depexSuccessCodes?.getRepositoriesSuccess || 'Repositories retrieved successfully',
    'get_package_status_success': translations.depexSuccessCodes?.getPackageStatusSuccess || 'Package status retrieved successfully',
    'get_version_status_success': translations.depexSuccessCodes?.getVersionStatusSuccess || 'Version status retrieved successfully',
    'version_initializing': translations.depexSuccessCodes?.versionInitializing || 'Version initialization started',
    'package_initializing': translations.depexSuccessCodes?.packageInitializing || 'Package initialization started',
    'init_repo': translations.depexSuccessCodes?.initRepo || 'Repository initialization started',
  }

  return successCodes[code] || translations.authSuccessCodes?.unknownSuccess || 'Operation completed successfully'
}
...
export function getDepexErrorMessage(code: string, translations: Record<string, any>): string {
  ...
  return depexErrorCodes[code] || translations.depexApiErrors?.networkError || 'Unknown error occurred'
}
...
export function getDepexSuccessMessage(code: string, translations: Record<string, any>): string {
  ...
  return depexSuccessCodes[code] || translations.depexApiSuccess?.operationCompleted || 'Operation completed successfully'
}
