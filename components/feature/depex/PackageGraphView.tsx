...
export default function PackageGraphView({
  open,
  onOpenChange,
  packageName,
  purl,
  ...
  const { graph, expandNode, collapseNode, fetched, loadingNodes } = usePackageGraph({
    packageName,
    purl,
    nodeType,
  })
  ...
  const graphStats = useMemo(() => {
    ...
    const nodesWithVulnerabilities = displayedNodes.filter(
      node => node.props?.vulnerabilities && node.props.vulnerabilities.length > 0
    ).length

    const totalVulnerabilities = displayedNodes.reduce(
      (sum, node) => sum + (node.props?.vulnerabilities?.length || 0),
      0
    )
        ...
        if (!graphRef.current && containerRef.current) {
          try {
            ...
            Graph.width(width)
              ...
              .linkColor((_link: any) => '#64748b')
              .linkWidth((_link: any) => 2)
        ...
        {isNodeLimitReached && (
          <div className="mx-4 mt-4 p-3 bg-destructive/10 border border-destructive/20 rounded-md">
            <p className="text-sm text-destructive">
              <strong>Node limit reached:</strong> You have reached the maximum of {MAX_NODES}{' '}
              nodes. Collapse some nodes to expand others.
            </p>
          </div>
        )}
        ...
        <div className="flex-1 flex gap-4 p-4 min-h-0">
                    ...
                    <div className="space-y-2 pt-2 border-t">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-red-600">
                          {graphStats.nodesWithVulnerabilities}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-muted-foreground">Total Vulns:</span>
                        <span className="font-semibold text-red-600">
                          {graphStats.totalVulnerabilities}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="w-80 border-l p-4 overflow-auto bg-background">
            <h4 className="font-semibold mb-3">
              {selectedEdge ? 'Relationship properties' : 'Node properties'}
            </h4>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {selected.type === 'Version' && (
                    <>
                      ...
                      {selected.props?.vulnerabilities &&
                        selected.props.vulnerabilities.length > 0 && (
                          <div>
