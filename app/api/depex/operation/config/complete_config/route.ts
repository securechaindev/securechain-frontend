...
export async function POST(request: NextRequest) {
  ...
  return withAuth(request, async authToken => {
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/depex/operation/config/complete_config`, {
        method: 'POST',
        headers: {
      ...
      console.error('Complete config API error:', error)
  ...
  return withAuth(request, async authToken => {
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/depex/operation/config/complete_config?${searchParams}`,
        {
          method: 'GET',
          headers: {
            Authorization: authToken,
      ...
      console.error('Complete config GET API error:', error)
      return NextResponse.json(
        {
